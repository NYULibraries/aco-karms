	...
	for field in rec_fields:
		if not field.is_control_field() and (len(field.get_subfields('6')) > 0):	# the field contains a subfield $6
			...
			for subfield in field:
				...
				if needs_sorted and first_sub and subfield[0] == '6':
					...
					if first_sub:
						...
						#print 'Orig field: '+field.value()		# this shows what the original field looks like
			...
			if needs_sorted:
				...
				print 'New field 6 only: '+new_field.value()	# this is to show whether the $6 subfield is on the new_field object
				for sub in field_subs:
					# add the remaining subfields to the new_field object
					new_field.add_subfield(sub[0],sub[1])
				print 'Orig field: '+field.value()		# this shows what the original field looks like
				print 'New field: '+new_field.value()	# this shows what the new field looks like
				print '-----------------------'
				rec.remove_field(field)				# delete the original field from the record
				rec.add_ordered_field(new_field)	# add the new field to the record
