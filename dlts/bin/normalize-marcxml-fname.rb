...
def print_usage
  str = ["usage: #$0 <marcxml file 1> [<marcxml file 2> ...]",
    "   <marcxml file 1> : first  MARCXML file to rename using MARC 003, 001 fields",
    "   <marcxml file 2> : second MARCXML file to rename using MARC 003, 001 fields",
    "   ...              : additional MARCXML files to rename",
    " ",
    "example:",
    "   #$0 foo.xml bar.xml",
    " "]
...
files.each do |f|
  begin
    m = Marcxml.new(f)
    new_name = "#{m.get_003}_#{m.get_001}_marcxml.xml"
    valid += 1
    dir = File.dirname(f)
    FileUtils.mv f, File.join(dir, new_name), verbose: true
  rescue => e
    errors << "#{f} : #{e.message}"
    invalid += 1
  end
